  ...
  if (!employeeId) {
    return (
      <div className="text-center py-8 bg-white rounded-lg shadow">
        ...
        <h3 className="text-xl font-medium text-gray-600 mb-2">ID dipendente non valido</h3>
        <Link href="/employees">
          <a className="mt-4 inline-block text-[#70fad3] hover:underline">
            <i className="fas fa-arrow-left mr-2"></i> Torna a Dipendenti
          </a>
        </Link>
      </div>
    );
    ...
    <>
      <div className="mb-8">
        ...
        {isLoadingEmployee ? (
          ...
          <div className="text-red-500">Dipendente non trovato</div>
        )}
      </div>
      
      <div className="mb-6">
        <h3 className="text-xl font-medium text-dark mb-4">Appartamenti Correlati</h3>
        
        {isLoadingEmployee ? (
          <div className="text-center py-8">
            <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-[#70fad3]"></div>
            <p className="mt-2 text-gray-600">Caricamento appartamenti...</p>
          </div>
          ...
          <div className="text-center py-8 bg-white rounded-lg shadow">
            ...
            <h3 className="text-xl font-medium text-gray-600 mb-2">Nessun appartamento assegnato</h3>
            <p className="text-gray-500">
              Questo dipendente non ha ancora appartamenti assegnati.
            </p>
          </div>
        )}
      </div>
      ...
      <ConfirmDeleteModal
        ...
        message={`Sei sicuro di voler eliminare l'appartamento "${apartmentToDelete?.name}"?`}
      />
    </>
  );
}
