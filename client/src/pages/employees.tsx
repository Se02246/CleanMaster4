  ...
  return (
    <>
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h2 className="text-2xl font-semibold text-dark">Dipendenti</h2>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-8 bg-white rounded-lg shadow">
          ...
          <h3 className="text-xl font-medium text-gray-600 mb-2">Nessun dipendente trovato</h3>
          <p className="text-gray-500 mb-4">
            {searchQuery 
              ? "Nessun risultato corrisponde alla tua ricerca." 
              : "Non ci sono ancora dipendenti. Inizia aggiungendone uno!"}
          </p>
          <Button 
            onClick={handleOpenEmployeeModal}
            className="bg-[#70fad3] hover:bg-[#70fad3]/90 text-dark font-medium px-4 py-2 rounded-lg transition-colors"
          >
            <i className="fas fa-plus mr-2"></i> AGGIUNGI DIPENDENTE
          </Button>
        </div>
      )}
      ...
      <ConfirmDeleteModal
        ...
        message={`Sei sicuro di voler eliminare il dipendente "${employeeToDelete?.name}"?`}
      />
    </>
  );
}
